...
/// Every test holds `CaptureStateGate` (shared with the proxy suite) so parallel suites never stomp the shared
/// `TrafficCapture.shared` global state; assertions are scoped to a host no other writer uses.
@MainActor
@Suite(.serialized)
struct JSCaptureEndToEndTests {
    ...
    @Test
    func nothingIsCapturedWhileSessionIsPaused() async {
